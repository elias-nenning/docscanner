    ...
    <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm">
      <div className="flex items-start justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-zinc-600 mt-1">Account + appearance preferences.</p>
        </div>
      </div>

      <div className="mt-6 grid lg:grid-cols-2 gap-4">
        <div className="border border-zinc-200 rounded-2xl p-5">
          <div className="text-sm font-bold text-zinc-900">Account</div>
          <div className="text-sm text-zinc-600 mt-1">{user ? `Signed in as ${user.email}` : "Not signed in"}</div>
          ...
          <div className="mt-4 flex gap-2">
            {!isAuthed ? (
              ...
              <button
                ...
                className="bg-zinc-900 hover:bg-black text-white rounded-xl px-4 py-2 text-sm font-semibold transition"
              >
                Sign out
              </button>
            )}
        ...
        <div className="border border-zinc-200 rounded-2xl p-5">
          <div className="text-sm font-bold text-zinc-900">Appearance</div>
          <div className="text-sm text-zinc-600 mt-1">Choose light or dark mode.</div>

          <div className="mt-4 flex gap-2">
            <button
              onClick={() => setMode("light")}
              className={`flex-1 rounded-xl px-4 py-2 text-sm font-semibold border transition ${
                mode === "light" ? "bg-indigo-50 border-indigo-200 text-indigo-800" : "bg-white border-zinc-200 text-zinc-700 hover:bg-zinc-50"
              }`}
            >
              Light
            </button>
            <button
              ...
              className={`flex-1 rounded-xl px-4 py-2 text-sm font-semibold border transition ${
                mode === "dark" ? "bg-indigo-50 border-indigo-200 text-indigo-800" : "bg-white border-zinc-200 text-zinc-700 hover:bg-zinc-50"
              }`}
            >
              Dark
            </button>
          ...
          <div className="mt-4 flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-zinc-900">Reduce motion</div>
              <div className="text-xs text-zinc-500">Prefer fewer animations.</div>
            </div>
            <button
              onClick={() => update({ reduceMotion: !settings.reduceMotion })}
              className={`w-12 h-7 rounded-full border transition flex items-center px-1 ${settings.reduceMotion ? "bg-indigo-600 border-indigo-600" : "bg-zinc-100 border-zinc-200"}`}
        ...
        <div className="border border-zinc-200 rounded-2xl p-5">
          ...
          <div className="mt-4 space-y-3">
            ...
            <label className="flex items-center justify-between gap-4">
              <span className="text-sm text-zinc-700">Email updates</span>
        ...
        <div className="border border-zinc-200 rounded-2xl p-5">
          <div className="text-sm font-bold text-zinc-900">Language</div>
          <div className="text-sm text-zinc-600 mt-1">UI language preference.</div>
          <div className="mt-4">
            <select
              value={settings.language}
              onChange={(e) => update({ language: e.target.value as SettingsState["language"] })}
              className="w-full border border-zinc-200 rounded-xl px-3 py-2 text-sm bg-white"
            >
